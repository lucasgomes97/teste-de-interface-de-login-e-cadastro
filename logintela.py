...
class Toplevel1:
    ...
    def __init__(self, top=None):
        ...
        self.Button1_1 = tk.Button(self.Frame1, command=Cadastro)  #verificar função 
    ...
    def loginBackEnd(self):
        ...
        with open ("cpf.txt", 'r')as arquivosUsuario:
            cpf = arquivosUsuario.readlines()
            cpf = list(map(lambda x: x.replace('\n',''),cpf))
        ...
        for i in range(len(usuarios)):
            if usuario == usuarios[i] and senha == cpf [i]:
...
class Cadastro:
     ...
     def cadastrarBackEnd(self):
        try:
            with open ("usuarios.txt", 'a')as arquivosUsuario:
                arquivosUsuario.write(self.Entry_nome.get()+ '\n')
            
                with open ("cpf.txt", 'a')as arquivosUsuario:
                    arquivosUsuario.write(self.Entry_cpf.get()+ '\n')
                self.root.destroy()
        except:
            print("Houve um erro")
